...
TRADE_TABLE_NAME = "raw_rent" # 저장할 테이블
...
def parse_rent_xml_to_df(xml_text: str) -> pd.DataFrame:
    ...
    total_count_str = root.findtext('.//totalCount', '0')
    try:
    ...
    for item in items:
        ...
        record = {
            ...
            '월세': rent_str,
            '계약일': deal_date,
            '계약유형': item.findtext('contractType')  # (예시)
        }
...
# --- 2. API 호출 및 저장 함수 ---
def fetch_rent_data_and_save(lawd_cd: str, deal_ymd: str) -> bool:
    # 1. API 요청 파라미터 설정
    params = {
    ...
    try:
        # 2. API 호출
        response = requests.get(API_URL_RENT, params=params, timeout=30)
        # 2xx (정상) 상태 코드가 아니면 예외 발생
        response.raise_for_status()

        # 3. XML 텍스트를 파싱 함수로 전달
        xml_response_text = response.text
        df_api_data = parse_rent_xml_to_df(xml_response_text)
    ...
    try:
        ...
        while call_count < API_CALL_LIMIT_PER_RUN:
            ...
            for region_code, last_fetched_date_str in regions:
                ...
                # 7. 실제 API 호출
                success = fetch_rent_data_and_save(region_code, date_ym_str)
